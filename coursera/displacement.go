...
func main() {
	acceleration, velocity, initialDisplacement := readUserInput()
	fmt.Println(acceleration, velocity, initialDisplacement)
	fn := GenDisplaceFn(acceleration, velocity, initialDisplacement)
	fmt.Println("After 3secs: ", fn(3))
	fmt.Println("After 5secs: ", fn(5))
...
func GenDisplaceFn(a float64, vo float64, so float64) func(t float64) float64 {
	...
	return func(t float64) float64 {
		displacement := (a*math.Pow(t, 2))/2 + (vo * t) + so
		return displacement
	}
}

func readUserInput() (acceleration, velocity, displacement float64) {
